...
class Common {
     ...
     * @returns Wallet address
     */
    initializeEthWalletAndProvider(walletAddress, ethNetworkProvider) {
        if (!this.ethWalletAddress)
            this.ethWalletAddress = walletAddress;
        if (!ethNetworkProvider)
            throw Error("Ethereum Network Provider not provided in parameter");
        this.web3 = new web3_1.default(ethNetworkProvider);
     ...
     * @returns balance in ether
     */
    signPayloadEth(data, ethPrivateKey) {
        if (!this.web3) {
            throw Error("Ethereum Wallet and Network not initialized");
        }
        if (!ethPrivateKey) {
     ...
     * @returns ethereum wallet
     */
    getEthWalletByPrivateKey(ethPrivateKey) {
        if (!this.web3) {
            throw Error("Ethereum Wallet and Network not initialized");
        }
        if (!ethPrivateKey) {
    ...
    async _readContract() {
        try {
            const response = await axios_1.default.get(this.bundlerUrl + "/state/current");
            if (response.data)
                return response.data;
        }
        catch (e) {
    ...
    async readContractFromKYVE() {
        ...
        try {
            ...
            const computedStateFromSnapshot = await query_1.readContract(poolID, this.contractId, false);
